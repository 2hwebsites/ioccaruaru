...
define('TESTE', create_error('Entrou aqui.'));
...
*/
	...
	function Open()
	{
		...
		if($this->connectionId->connect_error){
			define('erro_conn', create_error(trigger_error("ERRO NA CONEXÃO: "  . $this->connectionId->connect_error, E_USER_ERROR)));
			echo erro_conn;
		}
	...
	function Close()
	{
		if ( $this->isOpen) //is_resource($this->connectionId) &&
		{
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId)){ // && is_resource($this->connectionId)
			...
			if (mysqli_num_rows($results) > 0){ //(is_resource($results) && 
	...
	*/
	...
	function GetDatabaseList()
	{
		...
		if (isset($this->connectionId) ){ //&& is_resource($this->connectionId)
				...
				$dbList = mysqli_query($this->connectionId, $query);
				
				while ($row = mysqli_fetch_row($dbList))
				{					
					$db = $row[0];
					$xmlOutput .= '<ROW><VALUE>' . $db . '</VALUE></ROW>';
				}
    }else{
