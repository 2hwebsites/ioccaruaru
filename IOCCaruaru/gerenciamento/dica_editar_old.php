<?php	
	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=index.php?p=dicas&cat=2'>";
	}

	include "../connections/conn.php";

	$res_dica = mysqli_fetch_assoc(mysqli_query($conn, "SELECT titulo, subtitulo, thumb, nao_exibir_thumb, thumb_banner, video, texto
															From ioc_posts WHERE id = '".$id."'"));
	
	mysqli_close($conn);
	
	$titulo = $res_dica['titulo'];
	$subtitulo = $res_dica['subtitulo'];
	$thumb = $res_dica['thumb'];
	$thumbDemo = "../upload/dicas-saude/".sprintf("%06d", $id)."/img_p/".$thumb;
	$thumbBanner = $res_dica['thumb_banner'];
	$video = $res_dica['video'];
	$texto = $res_dica['texto'];
	
	if($res_dica['nao_exibir_thumb'] == '1'){
...
<script>
	...
	<form onSubmit="return critica(this);" action ="" method="post" enctype="multipart/form-data" name="dica_editar" 
    	id="dica_editar">
    	<fieldset>
        	<legend>Editar Dica de Saúde</legend>
            ...
            <label class="full lblInputFile">
            	<strong class="legendaCampos">Imagem de Capa<span class="alertaLegenda">*Apenas arquivos .jpg, .png e .gif</span>
                </strong>
            ...
            <label id="lblImgCapaDemo" class="lblImgBannerDemo">
             ...
             <label class="full halfLabel">
             	<input type="checkbox" id="nao_exibir_thumb" name="nao_exibir_thumb" value="1" <?php echo $naoExibirThumb; ?> /><span class="check">Não exibir imagem de capa na reportagem</span>
             </label>
