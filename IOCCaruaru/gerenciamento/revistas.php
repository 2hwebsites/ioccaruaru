<?php
	...
	if(@$_GET['acao'] == '3'){
		...
		mysqli_close($conn);
		redireciona("index.php?p=revistas&cat=6&us=".$usuario['id']);
	}
	...
	$revistasSql = "SELECT ip.id, ip.titulo, iu.nome_resumido
	...
	$revistasQy  = mysqli_query($conn, $revistasSql) or die(mysqli_error($conn));
	
	mysqli_close($conn);

	if (@mysqli_num_rows($revistasQy) == '0'){
		echo "<br>Nenhuma revista encontrada!";
	}else{
?>
...
<?php
		while($res_revistasQy = mysqli_fetch_array($revistasQy)){
			$id = $res_revistasQy[0];
			$titulo = $res_revistasQy[1];
			$nome = $res_revistasQy[2];
...
?>
  <tr bgcolor="<?php echo $cor; ?>" class="lista-conteudo">
    ...
    <td width="300" align="left" valign="middle">
    	<a href="<?php echo "index.php?p=revista_editar&id=".$id."&us=".$usuario['id']; ?>"><?php echo $titulo; ?></a>
	</td>
...
<div style="clear:both; padding-top:30px;">
	<center><?php echo paginacao($page, $total, "index.php?p=revistas&cat=6&us=".$usuario['id']."&pg="); ?></center>
</div>