<?php	
	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		if(empty($naoExibirThumb) && isset($_POST['nao_exibir_thumb'])){
			$campos[$cont] = "nao_exibir_thumb";
			$valores[$cont] = "'".$_POST['nao_exibir_thumb']."'";
			$cont++;
		} elseif(!empty($naoExibirThumb) && !isset($_POST['nao_exibir_thumb'])){
			$campos[$cont] = "nao_exibir_thumb";
			$valores[$cont] = "'0'";
			$cont++;
		}
		...
		switch ($_POST['destaque_post']){
			case "selecione":
				if(!empty($thumbBannerSelecionado)){
					$campos[$cont] = "thumb_banner";
					$valores[$cont] = "''";
					$cont++;
				}
				
				if(!empty($videoSelecionado)){
					$campos[$cont] = "video";
					$valores[$cont] = "''";
					$cont++;
				}
				...
				break;
			case "banner":
				if(!empty($_FILES['thumb_banner']['name']) || !empty($selecione)){
					$campos[$cont] = "thumb_banner";
					$valores[$cont] = "'".date("dmyHis").$_FILES['thumb_banner']['name']."'";
					$cont++;
					
					if(isset($thumbBanner)){
						$thumbBannerAntigo = $thumbBanner;
					}
			...
			case "video":
				...
				if(!empty($_POST['video']) || !empty($selecione)){
					$campos[$cont] = "video";
		...
		if(isset($usuario)){
			if($usuario <> $_SESSION['usuario']['id']){
				...
				$valores[$cont] = "'".$_SESSION['usuario']['id']."'";
				$cont++;
			}
		}
		...
		if(!empty($campos)){
			if(AlterarDados($campos, $valores, "ioc_posts", $id)){
				if($alterarImagem){
					$path = "../upload/dicas-saude/".sprintf("%06d", $id);
					
					// Remove as imagens anteriores
					if(isset($thumbAntigo)){
				...
				if($alterarBanner){
					$path = "../upload/dicas-saude/".sprintf("%06d", $id)."/banner/";
		...
		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=index.php?p=noticias&cat=2'>";
	}
	
	include "../connections/conn.php";

	$res_noticia = mysqli_fetch_assoc(mysqli_query($conn, "SELECT titulo, subtitulo, thumb, nao_exibir_thumb, thumb_banner, video,
															texto, usuario_id From ioc_posts WHERE id = '".$id."'"));
	...
	$thumbDemo = "../upload/dicas-saude/".sprintf("%06d", $id)."/img_p/".$thumb;
	$thumbBanner = $res_noticia['thumb_banner'];
	$video = $res_noticia['video'];
	$texto = $res_noticia['texto'];
	$usuario = $res_noticia['usuario_id'];
	...
	if(!empty($thumbBanner)){
		...
		$thumbBannerDemo = "../upload/dicas-saude/".sprintf("%06d", $id)."/banner/".$thumbBanner;
	} elseif(!empty($video)){
...
<script>
	...
	<form onSubmit="return critica(this);" action ="" method="post" enctype="multipart/form-data" name="noticia_novo" id="noticia_novo">
    	<fieldset>
        	<legend>Editar Dicas de Saúde</legend>
            ...
            <label class="full lblTextarea">
                ...
                <textarea name="subtitulo" id="subtitulo" class="textarea full textarea txaTextarea"><?php echo $subtitulo; ?></textarea>
            </label>

            <label class="full lblInputFile">
            	<strong class="legendaCampos">Imagem de Capa<span class="alertaLegenda">*Apenas arquivos .jpg, .png e .gif</span></strong>
...
<?php } ?>
			...
			<input name="cadastrar" type="submit" id="cadastrar" value="Editar Dica" class="cadastrar_btn" />
        </fieldset>
    </form>